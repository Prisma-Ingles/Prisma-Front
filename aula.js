...
function renderPergunta() {
  ...
  if (quizOptions) {
    quizOptions.innerHTML = pergunta.options.map(opcao => `
      <button class="curso-quiz-option" type="button" data-correct="${opcao.correta}">
        <span class="curso-quiz-number">${opcao.letra}</span>
        <span class="curso-quiz-text">${opcao.texto}</span>
      </button>
    `).join("");
  }
...
function ativarOpcoes() {
  const options = document.querySelectorAll(".curso-quiz-option");
